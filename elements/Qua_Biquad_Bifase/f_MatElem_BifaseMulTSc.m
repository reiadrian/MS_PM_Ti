function [kt,fint,eps_new,phi_new,p_new,eps_fluct,phi_fluct,p_fluct,...
            ...
            m_sig_p,m_chi_p,m_V_p,m_sig_phi,m_chi_phi,m_V_phi,velflu_dyn,T3,T4,T5,velflu_n1]=...
...
%Terminos la velocidad de filtracion dinamica al tiempo "n+1" 
T3_1 =zeros(1,nPG);
T4_1 =zeros(1,nPG);
T5_1 =zeros(1,nPG);
...
for iPG = 1:nPG
    ...
    p_T4(:,iPG) = beta_factor2*N4*p_fluct;
    phi_T5 = beta_factor1*((GradPorMacro_dup'*(N4*(yy(1:2,1:4))')'));
...
%############################################################################################
    ...
    switch conshyp
        ...
        case {15,16,17} %Modelo bifasico con solido elastico (saturado) con metodo de la penalidad o multiplicadores de Lagrange
           ...
           [ct,BiotM,beta,PermK,sigmaE_new(:,iPG),sigmaT_new(:,iPG),...
               mflu_new(1,iPG),velflu_sta(:,iPG),mflu_n1(:,iPG),velflu_n1(:,iPG),T3_1(:,iPG),T4_1(:,iPG),T5_1(:,iPG)] = ...
            f_Rmap_Bif_ElastME(eps_new(:,iPG),phi_new(:,iPG),phi_new_up_n(:,iPG),...
            p_new(:,iPG),p_new_sigma(:,iPG),eps_n1(:,iPG),phi_new_up_n1(:,iPG),...
            p_n1(:,iPG),e_DatMatSet,theta,eps_fluct(:,iPG),p_T4(:,iPG),phi_T5); 
        ...
        case {50,55}  %Modelo multiescala medio poroso saturado  
            [e_TanOp,sigmaE_new(:,iPG),sigmaT_new(:,iPG),mflu_new(:,iPG),...
                ...
                velflu_dyn(:,iPG),T3(:,iPG),T4(:,iPG),T5(:,iPG),velflu_n1(:,iPG)] = ...
    ...
    elseif conshyp==17  %Modelo bifasico con solido elastico (saturado) con metodo de 
        ...
        velflu_dyn(:,iPG) = -((mflu_new(:,iPG)/Dtime)*N4*(yy(1:2,1:4))')';
        T3(:,iPG) = -((T3_1(:,iPG)/Dtime)*N4*(yy(1:2,1:4))')';
        T4(:,iPG) = -((T4_1(:,iPG)/Dtime)*N4*(yy(1:2,1:4))')';
        T5(:,iPG) = -((T5_1(:,iPG)/Dtime)*N4*(yy(1:2,1:4))')';
