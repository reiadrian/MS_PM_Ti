...
for iSet = 1:nSet
    ...
    velflu_dyn= e_VarEst_new(iSet).velflu_dyn;
    %Componentes de velocidad de filtracion  dinamica al tiempo "n+1"
    T3 = e_VarEst_new(iSet).T3;
    T4 = e_VarEst_new(iSet).T4;
    T5 = e_VarEst_new(iSet).T5;
...
%     for iElem = 1:nElem
        ...
        [m_Ke(:,:,iElem),m_Fint(:,iElem),eps_new(:,iElem),phi_new(:,iElem),...
            ...
            velflu_dyn(:,iElem),T3(:,iElem),T4(:,iElem),T5(:,iElem),velflu_n1(:,iElem)]=...
    ...
    if e_VG.conshyp==50 || e_VG.conshyp==60 || e_VG.conshyp==61  || (~isfield(e_VG,'conshypMacro') && e_VG.conshyp==14)
        %############################################################################################
        ...
        e_VarEst_new(iSet) = f_OutVar_time(e_VarEst_new(iSet),e_VarEst_old(iSet),...
            ...
            mflu_new,velflu_sta,velflu_total,velflu_new,velflu_dyn,T3,T4,T5,velflu_n1,e_VG,0);
        %############################################################################################
    ...
    elseif isfield(e_VG,'conshypMacro') 
        ...
        e_VarEst_new(iSet) = f_OutVar_time(e_VarEst_new(iSet),e_VarEst_old(iSet),...
            eps_new,phi_new,porpr_new,eps_fluct,phi_fluct,p_fluct,sigmaE_new,sigmaT_new,...
            mflu_new,velflu_sta,velflu_total,velflu_new,velflu_dyn,T3,T4,T5,velflu_n1,e_VG,2);
        %############################################################################################        
    end
    %Variables historicas de cada modelo constitutivo al tiempo "n+1"   
