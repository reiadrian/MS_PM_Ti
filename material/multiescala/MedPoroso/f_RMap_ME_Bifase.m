...
function [e_TanOp,sigmaE_new,sigmaT_new,mflu_new,velflu_sta,velflu_total,velflu_new,...
    hvar_newMacro,velflu_dyn,T3,T4,T5,velflu_n1] = f_RMap_ME_Bifase(eps_new,phi_new_dup,phi_new_up_n,...
...
%Velocidad de filtracion al paso de tiempo "n+theta" 
velflu_n1 = f_HomogArea({e_VarEst_new.velflu_n1},2,omegaMicro_d,{e_DatSet.m_DetJT_d},e_DatSet,e_VG);
velflu_dyn = f_HomogArea({e_VarEst_new.velflu_dyn},2,omegaMicro_d,{e_DatSet.m_DetJT_d},e_DatSet,e_VG);
%Velocidad de filtracion al paso de tiempo "n+theta" 
T3 = f_HomogArea({e_VarEst_new.T3},2,omegaMicro_d,{e_DatSet.m_DetJT_d},e_DatSet,e_VG);
%Velocidad de filtracion al paso de tiempo "n+theta" 
T4 = f_HomogArea({e_VarEst_new.T4},2,omegaMicro_d,{e_DatSet.m_DetJT_d},e_DatSet,e_VG);
%Velocidad de filtracion al paso de tiempo "n+theta" 
T5 = f_HomogArea({e_VarEst_new.T5},2,omegaMicro_d,{e_DatSet.m_DetJT_d},e_DatSet,e_VG);
%##############################################################################################
...
for iSet = 1:e_VG.nSet
    ...
    e_VarEst_new(iSet) = f_OutVar_time(e_VarEst_new(iSet),e_VarEst_old(iSet),...
            ...
            e_VarEst_new(iSet).velflu_total,e_VarEst_new(iSet).velflu,0,0,0,0,0,e_VG,0);
    %############################################################################################
    %############################################################################################
    %VARIABLES MICRO-ESCALA EN EL PASO DE TIEMPO "n+1"
    %############################################################################################
